function loop2(jj, st, fin2, stf, finf, high, low, gsfact, dgs, cycles, dcy, days, dd, lrate1, dlr)
...
forw=0;
 rout='C:\Users\Nick\PycharmProjects\matlab_script\rtest12.csv';
...
fname="C:\Users\Nick\Downloads\for1.xls";  % change back to for1
rtfname="C:\Users\Nick\Downloads\testba.xlsx"; % day oanda data
...
tic;
%for cycles=1100:-50:900
%    for days=150:150
%        for cells=350:30:450
%            for lrate1=5:-1:3
%               lrate=lrate1*.001;
%                for gsfact1=7:2:7
%                    gsfact=.1*gsfact1;
                    for afile=stf:finf
                        ...
                        for k=st:fin2 % length(sn)-1
                            fmt='%1.0f %3.0f %3.1f %3.1f %3.5f\n';
                            fprintf(fmt,igpu,k,toc/60,toc/3600,jj-toc);
                            ...
                            win=0;
                            lose=0;
                            [win,lose,resout1]=tally(gsfact,m2,rout,fin2,snfai,afile,resout1,win,lose,st,k,cycles,days,lrate,cells,low,high);
        
                            fmt='%3.0f file=%2.0f win=%2.0f lose=%2.0f  %2.3f %3.3f %1.0f %4.0f days%3.0f lrate%0.3f cell%3.0f l %0.3f h %0.3f \n';
                            fprintf(fmt,k,afile,win,lose,100*(win-lose)/(win+lose),toc/3600,3,cycles,days,lrate,cells,low,gsfact);
                        end % st fin2 days
                    end % stf finf  files
%                end % gsfact
%            end % lrate1
...
%  
%
function [win,lose,resout1]=tally(gsfact,m2,rout,fin2,snfai,afile,resout1,win,lose,st,k,cycles,days,lrate,cells,low,high)
% long shortcounter
